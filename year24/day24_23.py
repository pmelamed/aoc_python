...
def task2( data: Data ) -> str:
    ...
    for node, vertex in data.graph.items():
        ...
        if len( node_largest ) > len( largest ):
            largest = node_largest
...
def get_largest_append( graph: Graph, added: set[ str ], candidates: list[ str ] ) -> set[ str ]:
    ...
    for index in range( len( candidates ) ):
        ...
        if can_append( graph, added, candidate ):
            ...
            if len( new_largest ) > len( largest ):
                largest = new_largest
...
def main():
    exec_tasks(
            ...
            task1,
            task2,
            read_file( '../data/samples/year24/day24_23.sample' ),
            7,
            "co,de,ka,ta"
    )
    exec_tasks(
            ...
            task1,
            task2,
            read_file( '../data/input/year24/day24_23.in' ),
            1400,
