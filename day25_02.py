...
def sum_mirrored_in_len(half_len: int, range_min: str, range_max: str) -> int:
    ...
    if min_hi < int(range_min[half_len:]): min_hi += 1
    max_hi = int(range_max[:half_len])
    if max_hi > int(range_max[half_len:]): max_hi -= 1
    if min_hi > max_hi: return 0
...
def main():
    exec_task(Data, task1, read_file('data/day25_02.sample'), 1227775554)
    exec_task(Data, task1, read_file('data/day25_02.in'), 32976912643)
    exec_task(Data, task2, read_file('data/day25_02.sample'), 4174379265)
    exec_task(Data, task2, read_file('data/day25_02.in'), 54446379122)
